...
- **Hydration time** — the SYNCHRONOUS hydration work in a headless browser, on
  a fresh page whose `#app` already holds the server-rendered DOM (timed in
  ...
  hydration *work*, not frame-scheduling latency. (React's `hydrateRoot` is
...
Dev numbers are misleading: unminified code plus the frameworks' *development*
...
`run.mjs [target] [iterations] [--no-build]` — `target` ∈ `{ripple-new, solid, react}`
(default `ripple-new`; a bare number is treated as iterations for back-compat). Each
run rebuilds the target's production client + SSR bundles unless `--no-build` is passed.
Build output goes to `<target>/dist/` (git-ignored).
...
never exercises. The React feed keeps the keyed `@for` (→ `.map(a => …key={a.id})`),
which passes each item by value, so `a.title` is direct.
