...
`<${Component} />` tag, where the tag-name expression tells the compiler the
component or element is dynamic. The runtime handles unwrapping tracked values
internally. This makes it straightforward to pass components as props or swap
them directly within a component, enabling flexible, state-driven UIs with
minimal boilerplate.
