...
describe('event-bundle optimization — {fn, args} hoisting + identity diff', () => {
  ...
  it('multi-arg bundle: per-arg slots + OR-chain identity diff on update', () => {
    ...
    expect(code).toMatch(/args:\s*\[\s*_b\._a\$\d+\$0\s*,\s*_b\._a\$\d+\$1\s*,\s*_b\._a\$\d+\$2\s*\]/);
    // Update: 4-way OR chain (fn + 3 args).
    expect(code).toMatch(/_b\._fn\$\d+\s*!==\s*_fn\s*\|\|\s*_b\._a\$\d+\$0\s*!==\s*_a0\s*\|\|\s*_b\._a\$\d+\$1\s*!==\s*_a1\s*\|\|\s*_b\._a\$\d+\$2\s*!==\s*_a2/);
  });
  ...
  it('per-row @for body: each row gets its own bundle capturing the iterated value', () => {
    ...
    expect(code).toMatch(/\.\$\$click\s*=\s*\{\s*fn:\s*_b\._fn\$\d+,\s*args:\s*\[\s*_b\._a\$\d+\$0\s*\]\s*\}/);
  });
