...
import {
  HookInIf, HooksInIfElse, RowList, TryWithHooks,
  DeclInIf, DeclInForLoop, DeclInTry,
...
describe('nested hooks', () => {
  ...
  it('per-item hooks survive reordering by key', () => {
    ...
    expect(r.findAll('button').slice(1).map(b => b.textContent)).toEqual(['a:0', 'b:0', 'c:0']);
    // Bump each row to a different count.
    r.click('.row-1 button'); r.click('.row-1 button'); r.click('.row-1 button');  // a:3
    r.click('.row-2 button');                                                       // b:1
    r.click('.row-3 button'); r.click('.row-3 button');                              // c:2
    expect(r.findAll('button').slice(1).map(b => b.textContent)).toEqual(['a:3', 'b:1', 'c:2']);
    // Reverse — same keys, state survives.
    r.click('#reverse');
    expect(r.findAll('button').slice(1).map(b => b.textContent)).toEqual(['c:2', 'b:1', 'a:3']);
    r.unmount();
  });
