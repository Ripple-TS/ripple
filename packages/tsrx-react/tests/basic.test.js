...
describe('@tsrx/react basic', () => {
	...
	it('leaves hooks after component-body guard returns as ordinary JavaScript', () => {
	...
	it('keeps rendered component-body guard branches as ordinary JavaScript', () => {
		...
		expect(code).toContain('useEffect(');
		expect(code).toContain("return <div>{'Foo not found'}</div>;");
		expect(code).toContain('return <div>{foo.trim()}</div>;');
		expect(code).not.toContain('App__Continue');
		expect(if_foo_mapping?.data.completion).toBe(true);
	});

	it('does not declare module helpers for hooks outside TSRX fragments', () => {
		const { code } = compile(
	...
	it('does not declare helper prop aliases for hooks outside TSRX fragments', () => {
		const { code } = compile_to_volar_mappings(
			`import { useEffect } from 'react';
