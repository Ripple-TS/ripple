...
// elements whose nesting `<outer><inner>x</inner></outer>` Chromium's parser
// does not create as written when parsed as template content (an ignored
// table part, a closed paragraph, a foster-parented text). The compiler must
// parse, not DOM-build, each of these (`tests/parser-nesting.test.js`).
// Needs a browser: run from the repository root with the benchmarks'
// Playwright, e.g.
...
await browser.close();
writeFileSync(out, JSON.stringify(pairs, null, '\t') + '\n');
console.log(`${pairs.length} rewritten pairs of ${tags.length} tags`);
