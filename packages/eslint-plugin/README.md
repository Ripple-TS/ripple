...
- Detects and uses `@ripple-ts/eslint-parser` if installed for `.tsrx` and `.ripple` files
...
- Excludes `.d.ts` files, `node_modules`, `dist`, and `build` directories from
  linting
- Works with both `.ts`/`.tsx` and Ripple component files (`.tsrx` by default, plus `.ripple`)
...
files (`.tsrx` by default, plus `.ripple`). In TypeScript modules, use `.value`
to read and write tracked values.
...
component files, including `.tsrx` and `.ripple`.
