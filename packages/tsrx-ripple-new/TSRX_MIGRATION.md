...
5. **`@for`** — the big one
   ...
   - Pass: `for.tsrx`, `continue.tsrx`, `nested.tsrx`, `benchmarks/ripple-new/Main.tsrx`.
...
> "Start Phase 3 step 1 (skeleton + component detection) on `tsrx-migration`. Reference `packages/ripple-new/TSRX_MIGRATION.md` for the build order. Don't touch fixtures outside the in-progress step; don't merge until step 4 bench gate passes."
