...
An `@if` (with an optional plain `@else`) whose branches are simple element templates, with only text and attribute updates and event handlers, now lowers to one block: the branch index and every branch's updates run in the same function, and the branch template is swapped only when the index changes. A selector key whose subscribers all left stays in the selector's map for reuse when the key comes back, and is swept once more than 1024 released keys pile up. A plain call with up to three scope-free arguments in a component is emitted as `scoped_call(block, fn, ...args)` instead of a `with_scope` thunk.
