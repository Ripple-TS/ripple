...
A selector key whose subscribers all left stays in the selector's map for reuse when the key comes back, and is swept once more than 1024 released keys pile up. A plain call with up to three scope-free arguments in a component is emitted as `scoped_call(block, fn, ...args)` instead of a `with_scope` thunk.
