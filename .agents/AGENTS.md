...
Ripple is a TypeScript UI framework that combines the best parts of React, Solid, and Svelte. Created by Dominic Gannaway ([@trueadm](https://github.com/trueadm)), Ripple is designed to be JS/TS-first with its own `.ripple` file extension that fully supports TypeScript.

## Documentation

For comprehensive Ripple syntax, components, reactivity, and API documentation, see:
- **[website/public/llms.txt](website/public/llms.txt)** - Full LLM-optimized documentation
...
1. **Package manager:** pnpm is required (`engines` in package.json enforces this)
2. **Internal code:** JavaScript with JSDoc type annotations (not TypeScript)
3. **Type definitions:** TypeScript `.d.ts` files in `types/` directories
4. **File naming:** kebab-case for files, PascalCase for components
5. **Consistency:** Look for similar implementations before adding new code
